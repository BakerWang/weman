	...
	<div style="background: #d7d7d7 none repeat scroll 0 0;margin-top:10px;">
		<div style="width:auto;font-size: 12px; border-bottom: 1px solid #ccc;border-top: 1px solid #ccc;cursor: default;">
			<table style="width:100%;font-size: 12px; " cellspacing="0" cellpadding="0" border="0">
				<tr class="datagrid-header"><td style="border-left: 1px solid #ccc;">id</td><td>评论内容</td><td>发文内容</td><td>创建者</td><td>创建时间</td><td>操作</td></tr>
				<s:iterator value="#request.page.result" var="articleCommentObj">
					<tr class="divTr" >
					<td style="border-left: 1px solid #ccc;">${articleCommentObj.id } </td>
					<td>${articleCommentObj.content }</td>
					<td>${articleCommentObj.articleContent }</td>
					<td>${articleCommentObj.username }</td>
					<td>
						<cc:dateFormat format="yyyy-MM-dd HH:mm" time="${articleCommentObj.create_time }"/>
					</td>
					<td>
						<select onchange="updateArticleCommentStatusSel(${articleCommentObj.id },this)">
							<option <s:if test="#articleCommentObj.status==-1">selected="selected"</s:if> value="-1">不能用</option>
							<option <s:if test="#articleCommentObj.status==1">selected="selected"</s:if> value="1">正在用</option>
						</select>
					</td></tr>
				</s:iterator>
			</table>
...
//            		$("#searchForm").submit();
	...
	function updateArticleCommentStatusSel(id,tt){
		var status = $(tt).val();
		$.ajax({
			type:'POST',
			url:'/b2b2cbak/apiAdmin/AdminArticleAction_updateArticleComment.do',
			data:{
				'status':status,
				'memberId':id
			},
	...
	function searchGoods(){
		...
		var url = '/b2b2cbak/apiAdmin/AdminArticleAction_fetchArticleCommentList.do?page.pageSize=1&adminSearchForm.title='+kw;
