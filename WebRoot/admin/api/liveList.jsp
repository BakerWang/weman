	...
	<div style="background: #d7d7d7 none repeat scroll 0 0;margin-top:10px;">
		<div style="width:auto;font-size: 12px; border-bottom: 1px solid #ccc;border-top: 1px solid #ccc;cursor: default;">
			<table style="width:100%;font-size: 12px; " cellspacing="0" cellpadding="0" border="0">
				...
				<td style="border-left: 1px solid #ccc;">id</td><td>房间名字</td><td>直播简介</td><td>回放封面</td><td>是否保存</td><td>创建时间</td><td>更新时间</td><td>操作</td></tr>
				<s:iterator value="#request.page.result" var="liveObj">
					<tr class="divTr" style="" height="100px" >
					<td style="border-left: 1px solid #ccc;">${liveObj.id } </td>
					<td>${liveObj.title }</td><td>${liveObj.details }</td>
					<td><img src="/b2b2cbak/statics/${liveObj.image }" width="80px" height="80px"/></td>
					<td><s:if test="#liveObj.isSave == 1">是</s:if><s:else>否</s:else>
					</td>
					<td>
						<cc:dateFormat format="yyyy-MM-dd HH:mm" time="${liveObj.createTime*1000 }"/>
					</td>
					<td>
						<cc:dateFormat format="yyyy-MM-dd HH:mm" time="${liveObj.startTime*1000 }"/>
					</td>
					<td>
						<a style="text-decoration: none;" class="b_fr" onclick="updateLive(${liveObj.id})" href="javascript:void(0);">修改</a>
					</td></tr>
				</s:iterator>
			</table>
		</div>
...
//            		var type = "${type}";
	...
	function updateLive(id){
		parent.addTab1('修改直播','/b2b2cbak/apiAdmin/AdminLiveAction_getLiveDetails.do?lid='+id);
	}
</script>