...
<div style="background-color: white; padding: 15px 0 8px 10px;margin:0px;position: relative;">
	<form id="addForm"  enctype="multipart/form-data"  method="post">
		<table style="width:98%;">
			<input type="hidden" value="${themeTag.id }" name="themeTag.id" />
			<tr height="38px"><td class="spanFront">标签类别: </td><td>
				<select id="pSel" name="themeTag.category" >
					...
					<s:iterator value="#request.parentTags" var="pt">
						<option <s:if test="#request.themeTag.category==#pt.id">selected="selected"</s:if> value="${pt.id }">${pt.name }</option>
					</s:iterator>
				</select>
...
<%-- 				</select> --%>
			...
			<tr height="38px"><td class="spanFront">标签名字 : </td><td>
				<input class="inputFront" value="${themeTag.name }" name="themeTag.name" />
			</td></tr>
...
<script>
	...
	function submitForm() {
			$.Loading.show("正在添加......");
			var options = {
				url : "/b2b2cbak/apiAdmin/AdminProductAction_updateThemeTagObj.do",
				type : "POST",
				dataType : 'json',
