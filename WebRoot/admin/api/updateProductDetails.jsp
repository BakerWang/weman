...
<div class='buttonArea'>
	<div style="background-color: white; padding: 15px 0 8px 10px; margin: 0px; position: relative; overflow: hidden;">
		<form id="addForm" enctype="multipart/form-data" method="post" style="width: 40%; float: left;">
			 	<input type="hidden" value="${product.goods_id }" name="productId" />
			 <table>
			 	<tr><td>商品来源：</td><td><select name="productOrigin">
			 	<option <s:if test="#request.product.productOrigin=='taobao'">selected="selected"</s:if> value="taobao">淘宝</option><option <s:if test="#request.product.productOrigin=='tmall'">selected="selected"</s:if> value="tmall">天猫</option><option <s:if test="#request.product.productOrigin=='jingdong'">selected="selected"</s:if> value="jingdong">京东</option><option <s:if test="#request.product.productOrigin=='yamaxun'">selected="selected"</s:if> value="yamaxun">亚马逊</option><option <s:if test="#request.product.productOrigin=='wemantb'">selected="selected"</s:if> value="wemantb">weman专场</option> </select> </td></tr>
			 	<tr><td>上架时间：</td><td><input class="easyui-datebox" name="startTime" style="width: 150px;height: 28px;" value="<cc:dateFormat format="yyyy-MM-dd HH:mm" time="${product.startTime }"/>" id="start_time" data-options="buttons:buttons" /></td></tr>
			 	<tr><td>下架时间：</td><td><input class="easyui-datebox" name="endTime" style="width: 150px;height: 28px;" value="<cc:dateFormat format="yyyy-MM-dd HH:mm" time="${product.endTime }"/>" id="end_time" data-options="buttons:buttons" /></td></tr>
			 	...
			 	<tr><td>商品图片：</td><td><input type="file" name="productImage"/><img width="100px" src="/b2b2cbak/statics/${product.original }"/></td></tr>
<!-- 			 	<tr><td>分类和品牌：</td><td> -->
...
<!-- 			 	</td></tr> -->
			 	<tr><td>APP里是否显示市场价格：</td><td>
			 		<select name="isShowMKPrice">
						<option <s:if test="#request.product.isShowMKPrice==1">selected="selected"</s:if> value="1">显示</option>
	...
	<div class="buttonWrap">
		<a href="javascript:;" onclick="submitForm()" class="l-btn" id="searchAdvance" ><span class="l-btn-left"><span class="l-btn-text">修改</span></span></a>
	</div>
</div>
...
// 		})
	...
	function submitForm() {
			$.Loading.show("正在添加......");
			var options = {
				url : "/b2b2cbak/apiAdmin/AdminProductAction_updateProduct.do",
				type : "POST",
				dataType : 'json',
				success : function(data) {
					if (data.result == "success") {
						//parent.addTab1("商品列表","/b2b2cbak/admin/storeGoods!list.do");
						parent.CloseTabByTitle("修改商品");
					}
				},
				error : function(e) {
					alert("出现错误 ，请重试"+e);
