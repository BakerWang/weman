...
public class StoreGoodsManager extends BaseSupport implements IStoreGoodsManager{
	...
	public List<Map> storeGoodsList(int storeid, Map map) {
		...
		if(map.get("is_groupbuy")!=null){
			sql.append(" and g.is_groupbuy!=1");
		}
...
//			}
	...
	public void saveGoodsStore(Integer storeid,Integer goods_id, Integer storeNum) {
		...
		if (storeid == 0) { // 新库存
			...
			// 更新库存
			this.daoSupport.execute("update es_product_store set store=?,enable_store=? where goodsid=?", storeNum,storeNum, goods_id);
		}
		
		this.daoSupport.execute("update es_goods set store=?,enable_store=? where goods_id=?", storeNum,storeNum, goods_id);
		this.daoSupport.execute("update es_product set store=?,enable_store=? where product_id=?", storeNum,storeNum, productid);
	}
	@Override
	@Transactional(propagation = Propagation.REQUIRED)
	public void saveGoodsSpecStore(Integer[] store_id,Integer goods_id, Integer[] storeNum,Integer[] product_id){
		for(int i= 0;i<store_id.length ;i ++){
			if(store_id[i] == 0) { //新库存
				...
				this.daoSupport.execute("update es_product_store set store=?,enable_store=? where storeid=?", storeNum[i],storeNum[i], store_id[i]);
			}
			//更新某个货品的总库存 
			this.daoSupport.execute("update es_product set store=?,enable_store=? where product_id=?", storeNum[i],storeNum[i], product_id[i]);
		}
		//更新商品总库存
		this.daoSupport.execute("update "+this.getTableName("goods")+" set store=(select sum(store) from "+this.getTableName("product_store")+" where goodsid=?),enable_store=store where goods_id=? ", goods_id,goods_id);
	}
