...
public class UploadUtil {
	...
	public static void createThumb(String sourcePath,String targetPath,int width){
		IThumbnailCreator thumbnailCreator = ThumbnailCreatorFactory.getCreator( sourcePath, targetPath);
		thumbnailCreator.resize(width);		
	}
	...
	public static String upload(File file,String fileFileName,String subFolder ) {
		...
		//String static_server_path= SystemSetting.getStatic_server_path();  //jack修改
		String static_server_path= ServletActionContext.getServletContext().getRealPath("/statics");  //jack修改 
		filePath =  static_server_path + "/attachment/";// static_server_path 是 D:/user/local/tomcat/....
		if(subFolder!=null){
			filePath+=subFolder +"/";
		}
		//原生返回的字符串
