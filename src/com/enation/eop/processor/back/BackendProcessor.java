...
public class BackendProcessor implements IEopProcessor {
	...
	public boolean process() throws IOException{
		...
		if( uri.startsWith("/admin/backendUi!login.do")){ //登陆界面放过
			return false;
		}
		
		if(uri.startsWith("/core/admin/adminUser!login.do")){//登陆验证放过
			return false;
		}
		...
		if(uri.startsWith("/admin") ){
			if(adminUser==null){
				...
				if(!uri.startsWith("/admin/backendUi")){//如果不是访问的登陆界，跳到登陆界面
